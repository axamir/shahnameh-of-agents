...
async function loadEvidence() {
  ...
  try {
    ...
    paths.forEach((item, index) => {
      ...
      link.target = "_blank";
      link.rel = "noreferrer";
      link.href = "https://github.com/axamir/echoes-consented-record/blob/main/" + item.path.split("/").map(encodeURIComponent).join("/");
      link.innerHTML = "<span>" + String(index + 1).padStart(2, "0") + " / PDF</span><b>" + escapeHtml(meta.echo) + "</b><p>" + escapeHtml(meta.label) + "</p><i>" + (meta.caseId ? "CASE " + escapeHtml(meta.caseId) : meta.date || "SOURCE RECORD") + "</i>";
      track.append(link);
    });
  } catch (error) {
